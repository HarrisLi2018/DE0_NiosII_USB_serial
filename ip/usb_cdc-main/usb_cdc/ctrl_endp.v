...
module ctrl_endp
  #(parameter VENDORID = 16'h0000,
   ...
   // Device Descriptor (in reverse order) for CDC class
...
//                                        }; // Standard Device Descriptor, USB2.0 9.6.1, page 261-263, Table 9-8
   // Device Descriptor (in reverse order) for keyboard
   localparam HID_DEV_DESCR_LENGTH = 18;
   localparam [8*HID_DEV_DESCR_LENGTH-1:0] HID_DEV_DESCR = {  8'h01, // bNumConfigurations
                                        8'h03, // iSerialNumber (no string)
                                        8'h02, // iProduct (no string)
                                        8'h01, // iManufacturer (no string)
                                        8'h02, // bcdDevice[1] (1.00)
                                        8'h00, // bcdDevice[0]
                                        8'h01, // idProduct[1] (keyboard)
                                        8'h01, // idProduct[0]
//                                        PRODUCTID[15:8], // idProduct[1]
//                                        PRODUCTID[7:0], // idProduct[0]
                                        VENDORID[15:8], // idVendor[1]
                                        VENDORID[7:0], // idVendor[0]
                                        CTRL_MAXPACKETSIZE[7:0], // bMaxPacketSize0
                                        8'h00, // bDeviceProtocol (keyboard)
                                        8'h00, // bDeviceSubClass (boot interface)
                                        8'h00, // bDeviceClass (Human Interface Device Class)
                                        8'h02, // bcdUSB[1] (2.00)
                                        8'h00, // bcdUSB[0]
                                        8'h01, // bDescriptorType (DEVICE)
                                        8'h12 // bLength
...
localparam HID_CONF_DESCR_LENGTH = 86;
    localparam [8*HID_CONF_DESCR_LENGTH-1:0] HID_CONF_DESCR = { // 512 bytes available
        ...
        8'h0A, // bInterval，描述 endpoint 傳輸間隔，此處為 100ms
        8'h00, // wMaxPacketSize[0]
        8'h40, // wMaxPacketSize[1]
        8'h03, // bmAttributes (interrupt)
        8'h82, // bEndpointAddress (Endpoint 1 IN)
        ...
        8'h00,
        8'h34, /* wItemLength: Total length of Report descriptor *///報告描述符長度**************
        8'h22, /* bDescriptorType *///下級描述符型別：0x22表示報告描述符
        8'h01, /* bNumDescriptors: Number of HID class descriptors to follow *///下級描述符數量
        8'h00, /* bCountryCode: Hardware target country *///國家程式碼
        8'h01, // bcdHID[0]
        8'h10,  /* bcdHID: HID Class Spec release number *///HID協定版本號 1.1
        8'h21, /* bDescriptorType: HID *///描述符型別：0x21表示HID描述符
        8'h09, /* bLength: HID Descriptor size *///描述符長度
        // HID descriptor
        
        8'h00, /* iInterface: Index of string descriptor *///介面描述符索引
        8'h02, /* nInterfaceProtocol : 0=none, 1=keyboard, 2=mouse *///該介面所使用的協定
        8'h01, /* bInterfaceSubClass : 1=BOOT, 0=no boot *///該介面所使用的子類
        8'h03, /* bInterfaceClass: HID *///該介面所使用的類
        8'h01, /* bNumEndpoints *///該介面使用的非0端點的端點數
        8'h00, /* bAlternateSetting: Alternate setting *///表示該介面的備用介面編號，從0開始遞增
        8'h00, /* bInterfaceNumber: Number of Interface *///表示該介面的編號，從0開始遞增
        8'h04, /* bDescriptorType: Interface descriptor type *///描述符型別：0x03表示介面描述符
        8'h09, /* bLength: Interface Descriptor size *///描述符長度
        // interface descriptor
        8'h32, /* MaxPower 100 mA: this current is used for detecting Vbus *///裝置需要的最大電流
        8'hC0, /* bmAttributes: Bus powered *//*Bus powered: 7th bit, Self Powered: 6th bit, Remote wakeup: 5th bit, reserved: 4..0 bits *///描述裝置特性
        8'h00, /* iConfiguration: Index of string descriptor describing the configuration*///設定字串索引
        8'h01, /* bConfigurationValue: Configuration value *///表示該設定集合的設定值，在設定設定請求中，主機會發送對應的設定值，表示設定了該設定
        8'h01, /* bNumInterfaces: 1 interface *///表示該設定所支援的介面數量
        8'h00,
        8'h22, /* wTotalLength: Bytes returned *///設定描述符集合總長度    ************
        8'h02, /* bDescriptorType: Configuration *///描述符型別：設定描述符爲0x02
        8'h09, /* bLength: Configuation Descriptor size *///描述符長度，9位元組
...
//                                         }; // Standard Configuration Descriptor, USB2.0 9.6.3, page 264-266, Table 9-10
   ...
   localparam CDC_DEV_DESCR_LENGTH = 'h12;
   localparam CDC_CONF_DESCR_LENGTH = 'h43;
   localparam CDC_LINE_CODING_LENGTH = 'd7;

   localparam [8*CDC_DEV_DESCR_LENGTH-1:0] CDC_DEV_DESCR = {
   ...
   localparam [8*CDC_CONF_DESCR_LENGTH-1:0] CDC_CONF_DESCR = {
                                         8'h00, // bInterval
                                         8'h00, // wMaxPacketSize[1]
                                         IN_BULK_MAXPACKETSIZE[7:0], // wMaxPacketSize[0]
                                         8'h02, // bmAttributes (bulk)
   ...
   localparam [8*CDC_LINE_CODING_LENGTH-1:0] CDC_LINE_CODING_3M = {
                                         8'd8,   // bDataBits
                                         8'd0,   // bParityType: none
                                         8'd0,   // bCharFormat: 1 stop bit
                                         8'h00,  // dwDTERate[31:24]
                                         8'h2D,  // dwDTERate[23:16]
                                         8'hC6,  // dwDTERate[15:8]
                                         8'hC0   // dwDTERate[7:0] = 3000000
                                         };
   ...
   localparam [3:0]        REQ_NONE = 4'd0,
                           ...
                           REQ_GET_LINE_CODING = 4'd12,
                           REQ_SET_LINE_CODING = 4'd13;
   localparam [1:0]        POWERED_STATE = 2'd0,
                           DEFAULT_STATE = 2'd1,
                           ADDRESS_STATE = 2'd2,
                           CONFIGURED_STATE = 2'd3;

   localparam              BC_WIDTH = ceil_log2(`max(CDC_CONF_DESCR_LENGTH,CDC_DEV_DESCR_LENGTH));
   ...
   reg                     in_req_q;
   reg                     usb_reset_q;
   reg [8*CDC_LINE_CODING_LENGTH-1:0] line_coding_q, line_coding_d;
   ...
   always @(posedge clk_i or negedge rstn) begin
      if (~rstn) begin
         ...
         addr_qq <= 7'd0;
         in_endp_q <= 1'b0;
         line_coding_q <= CDC_LINE_CODING_3M;
      end else begin
         if (clk_gate) begin
            state_q <= state_d;
   ...
   always @(/*AS*/addr_q or addr_qq or byte_cnt_q or class_q
            ...
            or in_dir_q or in_endp_q or in_req_q or line_coding_q or max_length_q
                 ...
                 case (byte_cnt_q)
                      ...
                      if (req_q == REQ_NONE) begin
                         if (class_q == 1'b0) begin
                            ...
                            if (dev_state_qq == CONFIGURED_STATE) begin
                               case (out_data_i)
                                 CLASS_REQ_SET_LINE_CODING : begin
                                    if (in_dir_q == 1'b0 && rec_q == REC_INTERFACE)
                                      req_d = REQ_SET_LINE_CODING;
                                 end
                                 CLASS_REQ_GET_LINE_CODING : begin
                                    if (in_dir_q == 1'b1 && rec_q == REC_INTERFACE)
                                      req_d = REQ_GET_LINE_CODING;
                                 end
                                 CLASS_REQ_SET_CONTROL_LINE_STATE,
                                 CLASS_REQ_SEND_BREAK : begin
                                    if (in_dir_q == 1'b0 && rec_q == REC_INTERFACE)
                                      req_d = REQ_DUMMY;
                                 end
                                 default : begin
                   ...
                   'd6 : begin // wLength LSB
                      ...
                      case (req_q)
                        ...
                        REQ_GET_LINE_CODING, REQ_SET_LINE_CODING : begin
                           if (out_data_i != 8'd7)
                             req_d = REQ_UNSUPPORTED;
                        end
                        REQ_DUMMY : begin
                   ...
                   'd7 : begin // wLength MSB
                      ...
                      case (req_q)
                        ...
                        REQ_GET_LINE_CODING, REQ_SET_LINE_CODING, REQ_DUMMY : begin
                           if (|out_data_i == 1'b1)
                             req_d = REQ_UNSUPPORTED;
                        end
                        REQ_GET_INTERFACE : begin
              ...
              if (in_dir_q == 1'b1) begin
                 if (out_valid_i == 1'b1) begin
                    ...
                    if (byte_cnt_q == max_length_q ||
                        (byte_cnt_q == CDC_DEV_DESCR_LENGTH && req_q == REQ_GET_DESCRIPTOR_DEVICE) ||
                        (byte_cnt_q == CDC_CONF_DESCR_LENGTH && req_q == REQ_GET_DESCRIPTOR_CONFIGURATION) ||
                        (byte_cnt_q == CDC_LINE_CODING_LENGTH && req_q == REQ_GET_LINE_CODING)) begin
                       ...
                       case (req_q)
                         ...
                         REQ_GET_DESCRIPTOR_DEVICE : begin
                            in_data = CDC_DEV_DESCR[8*byte_cnt_q +:8];
                            in_valid = 1'b1;
                         end
                         REQ_GET_DESCRIPTOR_CONFIGURATION : begin
                            in_data = CDC_CONF_DESCR[8*byte_cnt_q +:8];
                            in_valid = 1'b1;
                         end
                         REQ_GET_INTERFACE : begin
                            in_data = 8'd0;
                         ...
                         REQ_GET_LINE_CODING : begin
                            in_data = line_coding_q[8*byte_cnt_q +:8];
                            in_valid = 1'b1;
                         end
                         default :begin
                            in_data = 8'd0;
                 ...
                 if (in_req_q == 1'b1)
                   state_d = ST_STALL;
                    ...
                    if (req_q == REQ_SET_LINE_CODING && byte_cnt_q < CDC_LINE_CODING_LENGTH) begin
                       line_coding_d[8*byte_cnt_q +:8] = out_data_i;
                       byte_cnt_d = byte_cnt_q + 1;
                    end
                 end else
                   state_d = ST_STATUS;
